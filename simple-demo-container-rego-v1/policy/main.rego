...
deny contains msg if {
    ...
    msg = sprintf("Simple Demo Container Policy - Severity '%s': %v found (excluding allowed CWEs), exceeds threshold %v", [current_severity, vuln_count, severity_thresholds[current_severity]])
}
...
deny contains msg if {
    ...
    vuln_count > severity_thresholds[current_severity];
    msg = sprintf("Simple Demo Container Policy - Severity '%s': %v found (excluding allowed CWEs), exceeds threshold %v (any occurrence is a failure)", [current_severity, vuln_count, severity_thresholds[current_severity]])
}
